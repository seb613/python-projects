...
import Source.craft_text_detector.file_utils as file_utils
import Source.craft_text_detector.torch_utils as torch_utils
...
def load_craftnet_model(
    ...
    # load craft net
    from Source.craft_text_detector.models.craftnet import CraftNet
...
def load_refinenet_model(
    ...
    # load refine net
    from Source.craft_text_detector.models.refinenet import RefineNet
...
def adjustResultCoordinates(polys, ratio_w, ratio_h, ratio_net=2):
    if len(polys) > 0:
        ...
        filtered_polys = [p for p in polys if p is not None]
        
        # Scale only the valid polygons
        scaled_polys = []
        for poly in filtered_polys:
            scaled_polys.append(poly * (ratio_w * ratio_net, ratio_h * ratio_net))
        
        # Merge scaled polygons back, maintaining None values for unchanged indices
        for i, poly in enumerate(polys):
            if polys[i] is not None:
                polys[i] = scaled_polys.pop(0)
